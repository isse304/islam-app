...
export class FirebaseAuthService {
  ...
  async saveReadingHistory(location: { type: 'verse', surah: number, verse: number } | { type: 'page', page: number, surah: number }): Promise<void> {
    // ++ ADD LOG: When the method is called and with what location ++
    //console.log('[AuthService saveReadingHistory] Method called with location:', location);
    const user = this._user.getValue();
    if (!user) {
    ...
    try {
        ...
        const response = await firstValueFrom(this.http.post<any>(url, body).pipe(
            ...
            catchError(error => {
                ...
                return throwError(() => new Error('Failed to save reading history on server.'));
            })
        ));
        ...
        if (response?.success) {
            ...
            if (currentData) {
                ...
                // Remove any existing entry for the same surah
                const filteredHistory = updatedHistory.filter(entry => 
                    entry.surah !== body.surah
                );
                
                // Add new entry at the beginning
                filteredHistory.unshift(body);
                
                // Update the subject with new history
                this.userDataSubject.next({
                    ...currentData,
  ...
  async saveQuranReaderState(state: any): Promise<void> {
    ...
    try {
      ...
      // Save other state/preferences here if needed (e.g., last read position for quick resume)
      // The state object likely contains more than just surah/verse/page for history,
      // like reciter, translation, view mode, etc.
      // We should save these preferences, but NOT history via this method.
      
      // Example (assuming state object contains full preferences): 
      // await this.saveUserPreferences({ lastState: state }); // Adjust based on actual data structure
  ...
  async clearHistory(): Promise<void> {
    ...
    try {
        const response = await firstValueFrom( // Use firstValueFrom to await the observable
            this.http.delete<any>(
                `${environment.apiUrl}/api/user/${userId}/reading-history`
            ).pipe(
                timeout(this.API_TIMEOUT), // Add timeout
                retry(this.API_RETRY_ATTEMPTS), // Add retries
                catchError(error => {
                    console.error("[AuthService clearHistory] Error clearing history via API:", error);
                    // Rethrow the error to be caught by the outer try/catch
                    return throwError(() => new Error('Failed to clear reading history on server.'));
                })
            )
        );

        // Only update local state if server reported success
        if (response?.success) {
             //console.log("[AuthService clearHistory] Server confirmed history cleared.");
            const currentData = this.userDataSubject.getValue();
            if (currentData) {
                this.userDataSubject.next({ ...currentData, history: [] });
                // Also clear the cache for history
                localStorage.removeItem(`history_${userId}`);
            }
        } else {
            // If server responded but success is false, log and potentially throw
            console.error("[AuthService clearHistory] Server reported success: false for clearing history.", response);
            throw new Error('Server reported failure clearing history.');
        }

    } catch(error) {
        console.error("[AuthService clearHistory] Final error after server call or pipe:", error);
        // Re-throw error so calling component can handle it (e.g., show a message)
        throw error;
    }
  }
  ...
  removeBookmark(bookmark: string): Observable<BookmarkResponse> {
    return this.user$.pipe(
      ...
      switchMap(user => {
        ...
        // --- MODIFIED: Use the full bookmark string for the URL parameter ---
        // The server expects the full format (e.g., 'verse:6:3' or 'mushaf:11')
        const encodedBookmark = encodeURIComponent(bookmark);
        const url = `${environment.apiUrl}/api/user/${user.id}/bookmarks/${encodedBookmark}`;
        // --- End MODIFIED ---

        ////console.log(`[AuthService removeBookmark] Sending DELETE request to: ${url}`); // Log the constructed URL
        ...
        return this.http.delete<BookmarkResponse>(url).pipe(
          ...
          tap(response => {
            if (response.success) {
              // Update local cache/subject only after successful server save/delete
              const currentData = this.userDataSubject.value || {};
              const currentBookmarks = Array.isArray(currentData.bookmarks) ? currentData.bookmarks : [];
              this.userDataSubject.next({
                ...currentData,
                // Filter using the original bookmark string (e.g., 'verse:6:3') as that's what's stored locally
                bookmarks: currentBookmarks.filter(b => b !== bookmark)
              });
            } else {
                // If server returns success: false but no HTTP error, log or handle
                console.warn('[AuthService removeBookmark] Server reported success: false for deletion.', response);
                // Optionally re-fetch bookmarks here to ensure sync if needed
  ...
  addBookmark(bookmark: string): Observable<BookmarkResponse> {
    return this.user$.pipe(
      ...
      switchMap(user => {
        ...
        return this.http.post<BookmarkResponse>(url, body).pipe(
          timeout(this.API_TIMEOUT),
          retry(this.API_RETRY_ATTEMPTS),
          catchError(this.handleError),
          tap(response => {
  ...
  private async getReadingHistoryInternal(): Promise<ReadingHistoryEntry[]> {
    try {
      ...
      try {
        ...
        if (response && response.success && Array.isArray(response.history)) {
          // Update the userDataSubject with the fetched history
          const currentData = this.userDataSubject.getValue();
          if (currentData) {
            this.userDataSubject.next({
              ...currentData,
